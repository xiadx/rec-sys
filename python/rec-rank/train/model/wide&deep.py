    ...
    with open(test_file, "rb") as fp:
        ...
        for line in fp:
            if n == 0:
                n += 1
                continue
            if "?" in line.strip().decode("utf-8"):
                continue
            cls = line.strip().decode("utf-8").split(",")[-1]
            if cls == ">50K":
                labels.append(1)
            elif cls == "<=50K":
                labels.append(0)
...
def input(data_file, re_time, shuffle, batch_num, predict):
    ...
    def parse_csv(value):
        columns = tf.compat.v1.decode_csv(value, record_defaults=_CSV_COLUMN_DEFAUTL)
...
def main():
    ...
    preprocessing(os.path.join(job_path, "data/adult/adult.data"),
                  os.path.join(job_path, "data/adult/adult.test"),
                  os.path.join(job_path, "data/adult/train.data"),
                  os.path.join(job_path, "data/adult/test.data"))
    run(os.path.join(job_path, "data/adult/train.data"), os.path.join(job_path, "data/adult/test.data"), "wide&deep", "wide&deep_export")
