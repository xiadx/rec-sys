...
def save_libsvm_file(df, spark, feature_columns, label_columns, path, npartitions):
    ...
    rdd = df.rdd.map(lambda r: deal_sample(r)).repartition(npartitions)
...
def main():
    ...
    output_path = arg["output-path"].replace("{scene}.{model}.{version}", "%s.%s.%s" % (scene, model, version))
    ...
    ignore_cols = ignore + onehot_cols + vector_cols
    ...
    if dt_part:
        ...
        df_train = spark.read.parquet(*[os.path.join(input_path, day) for day in train_days_list]) \
            .select(*(sels + onehot_cols + vector_cols)) \
            .where(whe)
